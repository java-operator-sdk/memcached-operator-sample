...
@Controller(crdName = "memcacheds.cache.example.com")
public class MemcachedController implements ResourceController<Memcached> {
    ...
    public void init(EventSourceManager eventSourceManager) {
        ...
        eventSourceManager.registerEventSource("memcached-deployment",
            DeploymentEventSource.createAndRegisterWatch(client, deploymentLabel));
    }

    @Override
    public UpdateControl<Memcached> createOrUpdateResource(Memcached memcached, Context<Memcached> context) {
        Deployment deployment = client.apps().deployments().inNamespace(memcached.getMetadata().getNamespace())
                .withName(memcached.getMetadata().getName()).get();
        ...
        List<Pod> pods = client.pods().inNamespace(memcached.getMetadata().getNamespace())
        ...
        if (!CollectionUtils.isEqualCollection(podNames, memcached.getStatus().getNodes())) {
        ...
        return new DeploymentBuilder()
                .withMetadata(new ObjectMetaBuilder()
                        ...
                        .withOwnerReferences(new OwnerReferenceBuilder()
                                ...
                                .withKind("Memcached")
                                .withName(m.getMetadata().getName())
                                .withUid(m.getMetadata().getUid()).build())
                        .build())
                .withSpec(new DeploymentSpecBuilder()
                        .withReplicas(m.getSpec().getSize())
                        .withSelector(new LabelSelectorBuilder().withMatchLabels(labelsForMemcached(m)).build())
                        .withTemplate(new PodTemplateSpecBuilder()
                                .withMetadata(new ObjectMetaBuilder().withLabels(labelsForMemcached(m)).build())
                                .withSpec(new PodSpecBuilder().withContainers(new ContainerBuilder()
                                        ...
                                        .withPorts(new ContainerPortBuilder().withContainerPort(11211).withName("memcached").build()).build())
